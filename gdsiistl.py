...
#     sys.exit(0)
gdsii_file_path = "models/lineartrap-Laser_dicing.gds"
...
cells = [gdsii.cells["chip1"]] # get all cells that aren't referenced by another
...
for layer in layers:
    ...
    for index, (polygon, triangles, clockwise) in enumerate(layers[layer]):
        ...
        if len(ts) > 0:
            ...
            # print(top.shape)
            # faces = np.concatenate((top), axis=0)
        ...
        # print(faces.shape)
        # print(type(layer_mesh_data['vectors']))
        layer_mesh_data['vectors'][layer_pointer:(layer_pointer+len(top))] = top
        layer_pointer += len(top)

    # save layer to STL file
    filename = gdsii_file_path + '_{}.stl'.format(layername)
    print('    ({}, {}) to {}'.format(layer, layername, filename))
    layer_mesh_object = mesh.Mesh(layer_mesh_data, remove_empty_areas=True)
